...
/**
 * Turtles are a robotic device, which can break and place blocks, attack mobs, and move about the world. They have an
 * internal inventory of 16 slots, allowing them to store blocks they have broken or would like to place.
 *
 * ### Movement
 *
 ...
 *
 * {@link turtle.forward} and {@link turtle.back} move the turtle in the direction it is facing, while {@link turtle.up}
 * and {@link turtle.down} move it up and down (as one might expect!). In order to move left or right, you first need to
 * turn the turtle using {@link turtle.turnLeft}/{@link turtle.turnRight} and then move forward or backwards.
 *
 * #### INFO
 *
 * The name "turtle" comes from {@link https://en.wikipedia.org/wiki/Turtle_graphics Turtle graphics}, which originated
 * from the Logo programming language. Here you'd move a turtle with various commands like "move 10" and "turn left",
 * much like ComputerCraft's turtles!
 *
 * Moving a turtle (though not turning it) consumes _fuel_. If a turtle does not have any {@link turtle.refuel fuel}, it
 ...
 * functions, or wrap them in {@link assert} (for instance, use `assert(turtle.forward())` rather than
 ...
 * can be upgraded with _tools_ and peripherals. Turtles have two upgrade slots, one on the left and right sides.
 ...
 *
 * Turtle tools allow you to break blocks ({@link turtle.dig}) and attack entities ({@link turtle.attack}). Some tools are
 * more suitable to a task than others. For instance, a diamond pickaxe can break every block, while a sword does more
 * damage. Other tools have more niche use-cases, for instance hoes can til dirt.
 *
 * Peripherals (such as the {@link ModemPeripheral wireless modem} or {@link SpeakerPeripheral speaker}) can also be
 * equipped as upgrades. These are then accessible by accessing the `"left"` or `"right"` peripheral.
 *
 * | API                                                     | Description                                                                                                                         |
 * | ------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
...
declare interface Turtle {
    /**
     * Craft a recipe based on the turtle's inventory. The turtle's inventory should set up like a crafting grid. For
     * instance, to craft sticks, slots 1 and 5 should contain planks. _All_ other slots should be empty, including
     ...
     *
     * This requires a turtle tool capable of breaking the block. Diamond pickaxes (mining turtles) can break any
     * vanilla block, but other tools (such as axes) are more limited.
     *
     ...
     *
     * @since 1.4
     * @returns The current amount of fuel a turtle this turtle has or "unlimited" if turtles do not consume fuel when
     *   moving.
     ...
     *
     * {@link refuel} refuels the turtle, consuming fuel items (such as coal or lava buckets) from the currently selected
     * slot and converting them into energy. This finishes once the turtle is fully refuelled or all items have been
     * consumed.
     *
     * @since 1.4
     * @example
     ...
     *
     * @param [countA] The maximum number of items to consume. One can pass `0` to check if an item is combustable or
     *   not.
     ...
     */
    inspect(): LuaMultiReturn<[true, BlockData]> | LuaMultiReturn<[false, string]>;
     ...
     */
    inspectUp(): LuaMultiReturn<[true, BlockData]> | LuaMultiReturn<[false, string]>;
     ...
     */
    inspectDown(): LuaMultiReturn<[true, BlockData]> | LuaMultiReturn<[false, string]>;
     ...
     * @param [detailed] Whether to include "detailed" information. When `true` the method will contain much more
     *   information about the item at the cost of taking longer to run.
     * @returns Information about the given slot, or `undefined` if it is empty.
     * @throws If the slot is out of range.
     * @see {@link InventoryPeripheral.getItemDetail} Describes the information returned by a detailed query.
     */
    getItemDetail(slot?: number, detailed?: false): BasicDetailedItem | undefined;
    getItemDetail(slot: number | undefined, detailed: true): DetailedItem | undefined;

    /**
     * The builtin turtle API, without any generated helper functions.
     *
     * @deprecated Historically this table behaved differently to the main turtle API, but this is no longer the case.
...
declare type SuccessOrError = LuaMultiReturn<[true]> | LuaMultiReturn<[false, string]>;
