...
/**
 * Prints the specified values to the screen separated by spaces, wrapping if necessary. After printing, the cursor is
...
/**
 * Prints the specified values to the screen in red, separated by spaces, wrapping if necessary. After printing, the
...
/**
 * Reads user input from the terminal. This automatically handles arrow keys, pasting, character replacement, history
 * scrollback, auto-completion, and default values.
 *
 * @example
 ...
 *     let completion = require("cc.completion");
 ...
 *     print(msg);
 *
 * @param [replaceChar] A character to replace each typed character with. This can be used for hiding passwords, for
 *   example.
 * @param [history] A table holding history items that can be scrolled back to with the up/down arrow keys. The oldest
 *   item is at index 1, while the newest item is at the highest index.
 * @param [completeFn] A function to be used for completion. This function should take the partial text typed so far,
 *   and returns a list of possible completion options.
 ...
 *
 * This is a comma-separated list of settings pairs defined by the mod configuration or server owner. By default, it is
 * empty.
 *
 * An example value to disable autocompletion: `shell.autocomplete=false,lua.autocomplete=false,edit.autocomplete=false`
 *
 * @since 1.77
 * @example
